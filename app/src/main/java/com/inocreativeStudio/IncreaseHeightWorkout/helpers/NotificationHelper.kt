package com.inocreativeStudio.IncreaseHeightWorkout.helpers
...
class NotificationHelper(val ctx: Context) {
    ...
    private val CHANNEL_ONE_ID = "com.inocreativeStudio.IncreaseHeightWorkout.CHANNELONE"
    ...
    private fun shallNotify(): Boolean {
        ...
        var percent : Int =0;
        if(prefs.getInt(AppUtils.TOTAL_INTAKE, 1) !=0 )
            percent = sqliteHelper.getIntook(AppUtils.getCurrentDate()!!) * 100 / prefs.getInt(AppUtils.TOTAL_INTAKE, 1)
        else
            percent = sqliteHelper.getIntook(AppUtils.getCurrentDate()!!) * 100 / 1
